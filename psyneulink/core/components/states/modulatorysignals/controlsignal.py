...
class ControlSignal(ModulatorySignal):
    ...
    class Parameters(ModulatorySignal.Parameters):
        ...
        # isolated case
        variable = Parameter(np.array(defaultControlAllocation), aliases='allocation', getter=_output_state_variable_getter, pnl_internal=True)
        value = Parameter(np.array(defaultControlAllocation), read_only=True, aliases=['intensity'], pnl_internal=True, history_min_length=1)
